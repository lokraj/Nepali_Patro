...
def make_md(panchaanga, scripts=None, languages=None):
  ...
  # 📌 Add city and computation parameters
  print(get_computation_parameters_md(panchaanga, scripts), file=output_stream)

  # 📌 Add festival repo metadata
  if hasattr(panchaanga.computation_system, "festival_options"):
    print(panchaanga.computation_system.to_md(), file=output_stream)

    # 🎊 Print active festival repositories
    # if hasattr(panchaanga.computation_system.festival_options, "repos"):
    #   print("### Festival repositories used:", file=output_stream)
    #   for repo in panchaanga.computation_system.festival_options.repos:
    #     print(f"- 📂 {repo}", file=output_stream)
  ...
  for day_index, daily_panchaanga in enumerate(daily_panchaangas):
    ...
    (title, details) = day_details.day_summary(
      d=day_index,
    ...
    festival_md = day_details.get_festivals_md(
    ...
    if festival_md.strip():
      print("#### %s\n%s" % (
        names.translate_or_transliterate(text="utsavAH", script=scripts[0]),
        festival_md
      ), file=output_stream)
...
def get_computation_parameters_md(panchaanga, scripts):
  return '### Computation parameters\n- 🌏**%s** (%s)' % (
    ...
    panchaanga.city.get_transliterated_name(script=scripts[0])
  )
