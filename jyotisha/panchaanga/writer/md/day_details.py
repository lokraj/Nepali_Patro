...
def print_year_details(daily_panchaanga, output_stream, script):
  ...
  # 👑 Nepal Sambat = Gregorian Year - 879
  try:
    # Parse Gregorian date
    g_date_str = daily_panchaanga.date.get_date_str()
    yyyy, mm, dd = map(int, g_date_str.split("-"))
    gdt = g_date(yyyy, mm, dd)

    # Bikram Sambat (via library)
    bs_date = ndt.date.from_datetime_date(gdt)
    bs_line = f"- विक्रम सम्बत: {bs_date.year}-{bs_date.month:02d}-{bs_date.day:02d}"
    transliterate_and_print(bs_line, script=script, output_stream=output_stream)

    # Nepal Sambat (approx): BS year - 56
    ns_year = bs_date.year - 56
    ns_line = f"- नेपाल सम्बत: {ns_year}-{bs_date.month:02d}-{bs_date.day:02d}"
    transliterate_and_print(ns_line, script=script, output_stream=output_stream)

    # Bauddha Sambat: BS year - 543
    bs_year = bs_date.year - 543
    buddha_line = f"- बौद्ध सम्बत: {bs_year}-{bs_date.month:02d}-{bs_date.day:02d}"
    transliterate_and_print(buddha_line, script=script, output_stream=output_stream)

  except Exception as e:
    print(f"- Calendar conversion failed: {e}", file=output_stream)
