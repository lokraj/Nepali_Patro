...
class SolarFestivalAssigner(FestivalAssigner):
  ...
  def assign_anadhyayana_dvadashi_yoga(self):
    ...
    for d, daily_panchaanga in enumerate(self.daily_panchaangas):
      if daily_panchaanga.lunar_date.month.index in [4, 6, 8]:
        # Only check d+1 if it exists
        tithi_today = daily_panchaanga.sunrise_day_angas.tithi_at_sunrise.index
        tithi_tomorrow = None
        if (d + 1) < len(self.daily_panchaangas):
          tithi_tomorrow = self.daily_panchaangas[d + 1].sunrise_day_angas.tithi_at_sunrise.index

        if tithi_today == 12 or tithi_tomorrow == 12:
          for _nakshatra in [17, 22, 27]:
            self._assign_yoga(
              'anadhyAyaH~dvAdazI-yOgaH',
              [
                (zodiac.AngaType.NAKSHATRA, _nakshatra),
                (zodiac.AngaType.TITHI, 12)
              ],
              jd_start=daily_panchaanga.jd_sunrise - 1,
              jd_end=daily_panchaanga.jd_sunset + 2,
